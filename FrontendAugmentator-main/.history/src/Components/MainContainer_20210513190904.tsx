...
import { CheckAdminInterface } from "../Models/Interfaces";
...
export const MainContainer: React.FunctionComponent<{}> = () => {
  ...
  useEffect(() => {
      //iau jwt-ul si-l trimit la server
      const jwt = window.localStorage.getItem("token");
     //trimite request ca sa vad daca e admin
     const data : CheckAdminInterface = {
      token:jwt!
      }
     AccountService.checkAdmin(data)
         .then(resp => {
            setIsAdmin(resp)
         })
         .catch(err => {
  ...
  const drawer = (
    <div>
      ...
      <List>
        ...
        <ListItem button key="admin" onClick={() => history.push("/app/admin")}>
          <ListItemIcon>
