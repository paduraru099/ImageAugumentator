...
export default class AccountService{
   ...
   static checkAdmin = (token:string) : Promise<any> =>{
      console.log("Sunt in checkAdmin si primesc token " + token)
      let config = {
         headers: {
   ...
   static refreshToken = () : void =>{
      ...
      axios.post(`${AccountService.BASE_URL}/refresh`,{}, config).then(resp =>{
         window.localStorage.setItem('RefreshToken',resp.data.access_token)
      })
   }
